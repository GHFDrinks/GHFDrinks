...
const brands = [
  ...
  },
  {
    ...
    description:
      "Coates & Seely produces award-winning English sparkling wines from chalk-soil vineyards in Hampshire. Refined, classic methodology sparkling wines.",
    bcorp: false,
    promotion_active: false,
    variants: [
      { name: "Brut Reserve", volume: "75cl", abv: "12%", sort_order: 1 },
      { name: "Rosé Brut", volume: "75cl", abv: "12%", sort_order: 2 },
    ],
    activations: [],
  },
  {
    ...
    category: "Wines",
    tagline: "Single Quinta Port & Douro Wine.",
    description:
      "Quinta da Romaneira is one of the grandest and most historic estates in the Douro, producing world-class Port and still Douro wines.",
    bcorp: false,
    promotion_active: false,
    variants: [
      { name: "Tinto", volume: "75cl", abv: "14%", sort_order: 1 },
      { name: "Fine Ruby Port", volume: "75cl", abv: "19.5%", sort_order: 2 },
      { name: "10 Year Old Tawny Port", volume: "75cl", abv: "20%", sort_order: 3 },
    ],
    activations: [],
  },
  {
    ...
    name: "Dreamsake",
    category: "Wines",
    tagline: "Super-Premium Sake.",
    description:
      "Dreamsake is a super-premium Japanese sake brewed using ancient methods and pristine mountain waters, designed for modern dining experiences.",
    bcorp: false,
    promotion_active: false,
    variants: [
      { name: "Junmai Daiginjo", volume: "72cl", abv: "15%", sort_order: 1 },
    ],
    activations: [],
  },
  ...
  try {
    ...
    for (const brand of brands) {
      ...
      const { data: inserted, error: brandError } = await supabase
        .from("brands")
        .upsert({ ...brandData }, { onConflict: "slug" })
        .select()
        .single();

      if (brandError) {
        results.push({ slug: brand.slug, status: "error", message: brandError.message });
        continue;
      }

      if (variants.length > 0) {
        await supabase.from("brand_variants").delete().eq("brand_id", inserted.id);
        await supabase.from("brand_variants").insert(
      ...
      results.push({ slug: brand.slug, status: "success" });
    }
