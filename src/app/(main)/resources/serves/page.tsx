...
export default function ServeInspirationPage() {
  ...
  const handleBrandClick = (slug: string) => {
    if (typeof window !== "undefined") {
      sessionStorage.setItem("ghf_return_to", "/resources/serves");
      sessionStorage.setItem("ghf_return_label", "Back to Serve Inspiration");
    }
    router.push(`/serves/${slug}`);
  };
  ...
  return (
    <div className="min-h-screen px-12 py-10" style={{ backgroundColor: "var(--background)" }}>
      ...
      <div className="flex items-center justify-between mb-10 border-b border-[var(--border)] pb-6">
        <div>
          ...
          <p className="text-xs text-[var(--muted-foreground)] mt-1">
            Select a spirits brand to view signature seasonal recipes, tasting serves, and mixology guides
          </p>
        </div>
        <button
      ...
      {/* Brand Grid - Spirits Only */}
      <BrandTileGrid
        basePath="/serves"
        filter={(brand) => brand.category?.toLowerCase() === "spirits"}
