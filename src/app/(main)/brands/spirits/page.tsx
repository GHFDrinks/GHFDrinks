...
export default function SpiritsPage() {
  ...
  const brands = allBrands.filter(b => b.category.toLowerCase().includes("spirit"));
    ...
    <div className="space-y-16 pb-24 px-6 lg:px-12 pt-12">
      <header className="flex flex-col space-y-4">
        ...
        <RevealAnimation direction="up" delay={0.1}>
          <h1 className="text-6xl lg:text-8xl font-light tracking-tight">Spirits</h1>
        </RevealAnimation>
        <RevealAnimation direction="up" delay={0.2}>
          <p className="text-2xl lg:text-3xl text-muted-foreground max-w-4xl font-light leading-relaxed">
            Explore our curated selection of luxury spirits, crafted with climate-positive processes and premium ingredients.
          </p>
        </RevealAnimation>
      </header>
      ...
      {brands.length === 0 ? (
        <div className="py-24 text-center border border-white/10 rounded-3xl bg-white/5">
          <p className="text-xl text-white/50 font-light">No spirits have been published yet.</p>
        </div>
      ) : (
