...
export default function BeerCiderMixersPage() {
  ...
  useEffect(() => {
    async function loadBrands() {
      ...
      const filtered = data.filter(b => {
        ...
        return cat.includes("beer") || cat.includes("cider") || cat.includes("mixer") || cat.includes("low");
      });
      setBrands(filtered);
      setLoading(false);
    }
    ...
    <div className="space-y-16 pb-24 px-6 lg:px-12 pt-12">
      <header className="flex flex-col space-y-4">
        ...
        <RevealAnimation direction="up" delay={0.1}>
          <h1 className="text-6xl lg:text-8xl font-light tracking-tight">Beer, Cider & Mixer</h1>
        </RevealAnimation>
        <RevealAnimation direction="up" delay={0.2}>
          <p className="text-2xl lg:text-3xl text-muted-foreground max-w-4xl font-light leading-relaxed">
            Browse our range of artisan beers, ciders, and premium botanical mixers, including zero-proof options.
          </p>
        </RevealAnimation>
      </header>
      ...
      {brands.length === 0 ? (
        <div className="py-24 text-center border border-white/10 rounded-3xl bg-white/5">
          <p className="text-xl text-white/50 font-light">No products have been published yet.</p>
        </div>
      ) : (
