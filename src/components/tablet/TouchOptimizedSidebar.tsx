        ...
        {isOpen && (
          <>
            ...
            <motion.aside
              ...
              className="fixed inset-y-0 left-0 w-[80vw] max-w-sm bg-[var(--sidebar)] border-r border-[var(--border)]/20 flex flex-col z-[70] lg:hidden shadow-2xl"
            >
              <div className="h-24 flex items-center justify-between px-8 border-b border-[var(--border)]/20">
                <Link href="/admin" className="text-xl font-light tracking-widest uppercase text-[var(--foreground)]">
                  GHF<span className="text-accent ml-2">Admin</span>
                </Link>
                <button 
                  onClick={() => setIsOpen(false)}
                  className="w-12 h-12 -mr-3 rounded-full flex items-center justify-center text-[var(--muted-foreground)] active:bg-[var(--background)]/10 active:text-[var(--foreground)] transition-colors"
                >
                  <X className="w-6 h-6" />
                </button>
              </div>
                    ...
                    <Link key={item.name} href={item.href} onClick={() => setIsOpen(false)}>
                      <div className={cn(
                        ...
                        isActive 
                          ...
                          : "text-[var(--muted-foreground)] hover:bg-[var(--background)]/5 hover:text-[var(--foreground)]"
                      )}>
                        <Icon className={cn("w-6 h-6", isActive ? "text-accent" : "text-[var(--muted-foreground)]/65")} />
