...
function SortableBrandItem({ brand, onRemove }: { brand: Brand, onRemove: () => void }) {
  ...
  return (
    <div ref={setNodeRef} style={style} className="flex items-center space-x-4 bg-[var(--background)]/5 border border-[var(--border)]/20 rounded-2xl p-4 group">
      <button {...attributes} {...listeners} className="cursor-grab text-[var(--muted-foreground)]/60 hover:text-[var(--foreground)] transition-colors">
      ...
      <button onClick={onRemove} className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-[var(--background)]/10 text-[var(--muted-foreground)] hover:text-[var(--foreground)] transition-colors shrink-0">
        <X className="w-4 h-4" />
      </button>
    </div>
  );
...
export function PresentationBuilder() {
  ...
  return (
    <div className="flex h-full bg-background">
      ...
      <div className="w-[400px] border-r border-[var(--border)]/20 flex flex-col bg-[var(--background)]/[0.01]">
        <div className="p-6 border-b border-[var(--border)]/20">
          <input
            type="text"
            value={name}
            onChange={(e) => setName(e.target.value)}
            className="w-full bg-transparent text-2xl font-light focus:outline-none placeholder:text-[var(--muted-foreground)]/60 text-[var(--foreground)]"
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8 scrollbar-hide">
          ...
          <div>
            ...
            {selectedBrands.length === 0 ? (
              <div className="p-8 border border-dashed border-[var(--border)]/20 rounded-2xl text-center text-[var(--muted-foreground)] text-sm">
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-[var(--background)]/5 border border-[var(--border)]/20 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-accent transition-colors mb-4"
            />
            ...
            <div className="space-y-2">
              {filteredAvailableBrands.map(brand => (
                ...
                >
                  ...
                  <div className="w-8 h-8 rounded-full border border-[var(--border)]/20 flex items-center justify-center group-hover:bg-accent group-hover:text-black group-hover:border-accent transition-all">
                    <Plus className="w-4 h-4" />
                  </div>
                </button>
              ))}
              {filteredAvailableBrands.length === 0 && (
                <div className="text-center p-4 text-sm text-[var(--muted-foreground)]">No brands found.</div>
              )}
            </div>
          </div>
        </div>
        ...
        {/* Builder Actions */}
        <div className="p-6 border-t border-[var(--border)]/20 bg-black/20 backdrop-blur-md">
          <div className="flex space-x-3">
            <button 
              onClick={handleSave}
              disabled={selectedBrands.length === 0}
              className="flex-1 py-3 rounded-full border border-[var(--border)]/20 flex items-center justify-center space-x-2 hover:bg-[var(--background)]/10 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
      ...
      <div className="flex-1 bg-black p-8 lg:p-12 overflow-y-auto relative">
        <div className="max-w-5xl mx-auto">
          ...
          {selectedBrands.length === 0 ? (
            <div className="aspect-video rounded-[2rem] border border-dashed border-[var(--border)]/20 flex flex-col items-center justify-center text-[var(--muted-foreground)]/60">
              ...
              {selectedBrands.map((brand, bIdx) => (
                <div key={brand.id} className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    {/* Intro Slide */}
                    <div className="aspect-video rounded-2xl border border-[var(--border)]/20 overflow-hidden relative group bg-black">
                    ...
                    {brand.variants.length > 0 && (
                      <div className="aspect-video rounded-2xl border border-[var(--border)]/20 overflow-hidden relative group bg-[var(--muted)] p-6 flex flex-col justify-center">
                        <h4 className="text-sm text-[var(--muted-foreground)] uppercase tracking-widest mb-4">The Collection</h4>
                        ...
                        <div className="absolute top-3 left-3 px-2 py-1 bg-[var(--background)]/40 backdrop-blur-md rounded text-[10px] uppercase font-medium text-[var(--muted-foreground)]">Tasting & Variants</div>
                      </div>
                    )}
                  </div>
                </div>
