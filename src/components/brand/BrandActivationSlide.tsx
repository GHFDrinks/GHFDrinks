  ...
  return (
    <div className="flex flex-col h-screen w-full bg-white overflow-hidden">
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* LEFT COLUMN — badges */}
        <div className="flex-shrink-0 flex flex-col items-center justify-between py-6 border-r border-gray-100" style={{ width: "72px" }}>
          {act1?.activationType && (
        ...
        {act1 && (
          <div className="flex-1 flex flex-col border-r border-gray-100 overflow-hidden">
            <div className="overflow-hidden" style={{ height: "65%" }}>
              {act1Photo && <img src={act1Photo} alt={act1.title} className="w-full h-full object-cover" />}
            </div>
            <div className="flex-1 px-6 py-4 overflow-hidden">
              <p className="text-[10px] tracking-widest uppercase text-gray-400 mb-1">Activation</p>
              <h3 className="text-lg font-light mb-2 leading-tight" style={{ color: "var(--accent)" }}>{act1.title}</h3>
              <p className="text-xs text-gray-600 leading-relaxed mb-3 line-clamp-3">{act1.description}</p>
              {act1.keyDates?.length > 0 && (
                <div>
                  <p className="text-[10px] font-bold tracking-widest uppercase mb-1">Key Dates</p>
                  <p className="text-xs text-gray-700">{act1.keyDates.join(" | ")}</p>
                </div>
              )}
            </div>
          </div>
        ...
        {act2 && (
          <div className="flex-1 flex flex-col overflow-hidden">
            <div className="overflow-hidden" style={{ height: "65%" }}>
              {act2Photo && <img src={act2Photo} alt={act2.title} className="w-full h-full object-cover" />}
            </div>
            <div className="flex-1 px-6 py-4 overflow-hidden">
              <p className="text-[10px] tracking-widest uppercase text-gray-400 mb-1">Activation</p>
              <h3 className="text-lg font-light mb-2 leading-tight" style={{ color: "var(--accent)" }}>{act2.title}</h3>
              <p className="text-xs text-gray-600 leading-relaxed mb-3 line-clamp-3">{act2.description}</p>
              {act2.keyDates?.length > 0 && (
                <div>
                  <p className="text-[10px] font-bold tracking-widest uppercase mb-1">Key Dates</p>
                  <p className="text-xs text-gray-700">{act2.keyDates.join(" | ")}</p>
                </div>
              )}
            </div>
          </div>
      ...
      {tickerText && (
        <div className="h-8 flex items-center overflow-hidden border-t border-gray-200" style={{ backgroundColor: "var(--muted)" }}>
          <div className="flex animate-marquee whitespace-nowrap">
            <span className="text-[10px] tracking-widest uppercase text-gray-500 px-8">· {tickerText} · {tickerText} ·</span>
          </div>
        </div>
      )}
